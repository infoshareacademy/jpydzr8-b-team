...
class ZmianaDanych:
    ...
    def edycja_danych_uzytkownika(self, id, aktywnosc: bool = True):
        ''' Główna funkcja obiektu odbierająca polecenia.
        Jeśli nie następują błędy, które zawracają usera
        do Menu, poszczególne funkcje zbierają informacje
        do atrybutów obiektu, poczym z nich następuje
        zapis zmian w rekordzie danych, i aktualizacja
        bazy danych w txt.
        :return: None
        '''
        # prewencyjnie konwertuje na 'str'
        id = str(id).strip()
        ...
        # wgraj tymczasowo bazę do __LST_BAZA
        ZmianaDanych.__import_bazy()

        # uzyskaj rekord usera z bazy
        self.__indeks_rekordu = ZmianaDanych.__zwroc_nr_rekordu(dla_id=id)
        ...
        # gromadzenie danych przebiegło poprawnie
    ...
    def __menu_wybierz_dane(self) -> bool:
        ...
        while True:
            ...
            try:
                ...
                # cyfry poza zakresem - wróć do podmenu Dane ..
    ...
    def __wyrejestruj(self):
        ''' Wykasowanie rekordu i zapisanie go w plik Wyrejestrowani.
        :return: None
        '''
        self.komunikat(f'.. usuwam rekord {self.__indeks_rekordu}.', monit=False)
    ...
    def __zwroc_nr_rekordu(cls, dla_id: str, kolumna: int = 0) -> int | None:
        ''' Zwraca numer indeksu listy w zagnieżdżeniu.
        :param kolumna: przeszukiwany element rekordu wg layout'u
        :return: int | None - jesli nie znajduje
        '''
        # zwraca nr wiersza lub None, jeśli ID nie występuje
        x: int
        for i, linia in enumerate(cls.__LST_BAZA):
            try:
                x = linia.index(dla_id, kolumna, kolumna + 1)
                return i
            except:
                continue
        return None
    ...
    def __odpowiedz_z_menu(cls) -> str:
        ...
        for key, value in cls.__INDEKSY_REKORDU.items():
            if value in range(1, 9, 1):
                if value == 8:
                    key = 'wyrejestrowanie z biblioteki'
                tekst += f'\n\t{value} = {key}'
        tekst += linia
    ...
    # def czytaj_txt(cls):
    #     cls.__import_bazy()
    #     rec = cls.__LST_BAZA[2]
    #     with open('v_wyrejestrowani.txt', 'a') as f:
    #         if isinstance(rec, list):
    #             f.write(';'.join([str(item) for item in rec]) + '\n')
...
# def run(v=''):
#     edycja = ZmianaDanych()
#     edycja.edycja_danych_uzytkownika(v)
